...
class Toast {
    ...
    func present(title: String, symbol: String?, tint: Color = .primary, isUserInteractionEnabled: Bool = false, timing: ToastTime = .medium, position: Position = .bottom){
        DispatchQueue.main.async {
            self.position = position
            withAnimation(.snappy) {
                self.toasts.append(ToastItem(title: title, symbol: symbol, tint: tint, isUserInteractionEnabled: isUserInteractionEnabled, timing: timing))
            }
        }
    }
}
